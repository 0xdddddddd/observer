...
namespace test
{
    ...
    {
        using type = std::remove_pointer_t<typename function_bind_traits<T>::type>;
    };
    ...
    public:
        ...
        {
            using decayed_type = std::decay_t<_Fx>;
            if constexpr (std::is_function_v<std::remove_pointer_t<decayed_type>> ||
                          std::is_member_function_pointer_v<decayed_type>)
            {
                using cvref = function_traits_of<decayed_type>::type;
                printf("%s\n%s\n", typeid(decayed_type).name(), typeid(cvref).name());
                observers_[static_cast<std::size_t>(e)] = std::make_unique<observer<cvref>>(_Func);
            }
            else if constexpr (!std::is_bind_expression_v<decayed_type> && is_lambda_v<decayed_type>)
            {
                using cvref = function_lambda_traits_of<_Fx>::type;
                printf("%s\n%s\n", typeid(decayed_type).name(), typeid(cvref).name());
                observers_[static_cast<std::size_t>(e)] = std::make_unique<observer<cvref>>(_Func);
            }
            else if constexpr (std::is_bind_expression_v<decayed_type> && !is_lambda_v<decayed_type>)
            {
                using cvref = function_bind_traits_of<decayed_type>::type;
                printf("%s\n%s\n", typeid(decayed_type).name(), typeid(cvref).name());
                observers_[static_cast<std::size_t>(e)] = std::make_unique<observer<cvref>>(_Func);
            }
        }
